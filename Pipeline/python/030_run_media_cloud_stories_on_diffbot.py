...
print("030 Run stories on Diffbot")

runs = 0
while (1 == 1):
    cursor.execute("SELECT ID, Name FROM DiffBotBatch WHERE (StartTime IS NULL) AND (ID <> 1)") 
    rows = cursor.fetchall()
    ...
    runs += 1
    msg = "030 Run diffbot batches runs " + str(runs)
    print(msg)
